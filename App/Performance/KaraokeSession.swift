...
final class KaraokeSession: ObservableObject {
    ...
    func start(record: Bool = true) {
        ...
        Task {
            ...
            do {
                ...
                if position >= duration - 0.1 { seek(0);frames=[] }
                player?.volume=Float(accompanimentVolume)
                guard player?.play()==true else { throw ScoreError.malformedResponse }
                playing=true;hasMicrophone=granted
                status=granted ? "正在收音 · 手机外放含伴奏与回声" : "伴奏舞台 · 麦克风关闭"
                timer?.cancel()
    ...
    private func startMicrophone(token: Int) throws {
        ...
        guard format.sampleRate > 0,format.channelCount > 0 else { throw ScoreError.malformedResponse }
        ...
        probe.receive={ [weak self] frame in
            Task { @MainActor [weak self] in
                ...
                self.vocal=measured
                if self.frames.count<12000 { self.frames.append(measured) }
    ...
    func seek(_ value: Double) { position=min(duration,max(0,value));player?.currentTime=position }
    ...
    private func tick() {
        ...
        let target=bin<bins.count ? bins[bin] : 0.15
        energy=follow(energy,target,0.05,0.2)
        let beat=max(0,target-energy)*2
        let mood=currentCue?.mood ?? .reflective
        let rgb=mood.rgb
        ...
        if !preview && manager.canControl { manager.submitRhythmColor(light.color) }
    }
